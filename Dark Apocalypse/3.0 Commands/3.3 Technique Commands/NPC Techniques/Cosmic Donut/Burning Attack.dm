mob/var/learn_burning_attack = 0
...
mob/learn
	verb
		Burning_Attack()
			...
			Burning_Attack_Shoot()
			src.afk_time = 0

mob
	proc
		Burning_Attack_Shoot()
			var/obj/H = new/obj/burning_attack
			var/ki_damage = round(usr.powerlevel_max * 0.63)
			var/ki_cost = round(src.ki_max * 0.28)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Burning....."
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Burning Attack slams into you and you are blinded!"
										src <<"\red Your Burning Attack slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.sight |= BLIND
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your burning attack is Absorbed by [M]!"
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											src.doing = 0
											src.frozen = 0
											...
											M << "\red You try to absorb [src]'s Burning Attack, but it is too strong! You've been blinded"
											src <<"\red Your Burning attack slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Burning attack slams into you and you are blinded"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
								M.sight |= BLIND
