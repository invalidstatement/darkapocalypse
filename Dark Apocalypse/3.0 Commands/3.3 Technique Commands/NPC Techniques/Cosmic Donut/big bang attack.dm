...
mob
	proc
		Big_Bang_Shoot()
			var/obj/H = new/obj/big_bang
			var/ki_damage = input("How much power?") as num
			if(ki_damage < 0)
				src << "You must input more ki!"
				return
			if(src.powerlevel >= ki_damage)
				if(!src.doing)
					src.ki_lock = 1
					spawn(20) src.ki_lock = 0
					src.doing = 1
					src.powerlevel -= ki_damage
					src.overlays += kamehameha_charge
					view(6) << "<font color = white>[src]:</font> Get This!!!!"
					sleep(8)
					view(6) << "<font color = white>[src]:</font> Big..."
					sleep(8)
					view(6) << "<font color = white>[src]:</font> Bang..."
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s big bang attack slams into you!"
										src <<"\red Your big bang attack slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your big bang attack is Absorbed by [M]!"
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											src.doing = 0
											src.frozen = 0
											...
											M << "\red You try to absorb [src]'s big bang attack, but it is too strong!"
											src <<"\red Your big bang attack slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s big bang attack slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
