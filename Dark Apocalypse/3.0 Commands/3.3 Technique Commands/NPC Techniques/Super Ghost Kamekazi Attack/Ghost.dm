...
mob/learn/
	verb
		Super_Ghost_Kamikaze_Attack()
			...
			switch(alert("How Many?","Super Ghost Attacks","3","4","5"))
				if("3")
					Ghost_Shoot()
					Cont_Ghost()
					Cont_Ghost()
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Ghost slams into you!"
										src <<"\red Your Ghost slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Ghost is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Ghost, but it is too strong!"
											src <<"\red Your Ghost slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Ghost slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
