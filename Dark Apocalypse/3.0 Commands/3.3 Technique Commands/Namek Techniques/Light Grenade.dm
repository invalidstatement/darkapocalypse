...
mob
	proc
		Light_Grenade_Shoot()
			var/obj/H = new/obj/light_grenade
			var/ki_damage = round(usr.powerlevel_max * 0.65)
			var/ki_cost = round(src.ki_max * 0.60)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Light..."
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Light Grenade slams into you and you are blinded!"
										src <<"\red Your Light Grenade slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.sight |= BLIND
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											M << "\red You try to absorb [src]'s Light Grenade, but it is too strong! You've been blinded"
											src <<"\red Your Light Grenade slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Light Grenade slams into you and you are blinded"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
								M.sight |= BLIND
