obj/Blast2/Soul_Punisher
	icon = 'Soul Punisher.dmi'
	density = 1
	Bump(mob/M)
		if(istype(M,/mob))
			M.powerlevel=0
			M.BigKiDeathCheck2(M)
			del(src)

obj/Blast4/Soul_Punisher
	icon = 'arcb.dmi'
	density = 1
		...
		Brave_Cannon_Shoot()
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Brave Cannon!!!"
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Brave Cannon slams into you!"
										src <<"\red Your Brave Cannon slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Brave Cannon is Absorbed by [M]!"
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											src.doing = 0
											src.frozen = 0
											...
											M << "\red You try to absorb [src]'s Brave Cannon, but it is too strong!"
											src <<"\red Your Brave Cannon slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Brave Cannon slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
