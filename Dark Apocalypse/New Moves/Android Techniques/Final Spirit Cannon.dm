		...
		Final_Spirit_Cannon_Shoot()
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Final."
					sleep(8)
					view(6) << "<font color = white>[src]:</font> Spirit."
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Final Spirit Cannon slams into you!"
										src <<"\red Your Final Spirit Cannon slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Final Spirit Cannon is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Final Spirit Cannon, but it is too strong!"
											src <<"\red Your Final Spirit Cannon slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Final Spirit Cannon slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
