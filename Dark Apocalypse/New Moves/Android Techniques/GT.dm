		...
		Galatic_Tyrant_Shoot()
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Galatic Tyrant!!!"
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Galatic Tyrant slams into you!"
										src <<"\red Your Galatic Tyrant slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Galatic Tyrant is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Galatic Tyrant, but it is too strong!"
											src <<"\red Your Galatic Tyrant slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Galatic Tyrant slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
