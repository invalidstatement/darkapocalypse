...
mob/learn/
	verb
		Neo_Ki_Blast_Cannon()
			...
			switch(alert("How Many?","Blast Attacks","3","4","5"))
				if("3")
					Cannon_Shoot()
					Cont_Cannon()
					Cont_Cannon()
...
mob
	proc
		Cannon_Shoot()
			var/obj/H = new/obj/Cannon
			var/ki_damage = round(usr.powerlevel_max * 0.90)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					sleep(8)
					view(6) << "<font color = white>[src]:</font> ki..."
					sleep(8)
					view(6) << "<font color = white>[src]:</font> blast..."
					sleep(8)
					view(6) << "<font color = white>[src]:</font> Cannon..."
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s blast slams into you!"
										src <<"\red Your blast slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your blast is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s blast, but it is too strong!"
											src <<"\red Your blast slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s blast slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Cannon slams into you!"
										src <<"\red Your Cannon slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											M << "\red [src]'s attacks hits you, but you Absorb the energy!"
											src <<"\red Your Cannon is Absorbed by [M]!"
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											src.doing = 0
											src.frozen = 0
											...
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.powerlevel -= round(ki_damage)
											M << "\red You try to absorb [src]'s Cannon, but it is too strong!"
											src <<"\red Your Cannon slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Cannon slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
								M.powerlevel -= ki_damage
								M.BigKiDeathCheck(src)
							del(H)
