mob/var/cb_charge = 'CrusherBallCharge.dmi'
mob/var/cb_lock = 0

mob/learn
	verb
		Crusher_Ball(mob/M in oview(6))
			set name = "Crusher Ball"
			...
			var/ki_damage = round(((usr.ki_skill / usr.ki_skill_max) * usr.level) + usr.powerlevel_max * 20)
			var/ki_cost = round(src.ki_max * 2)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					src.doing = 1
					src.ki -= ki_cost
					src.overlays += cb_charge
					view(6) << "<font color = green>[src]:</font> Crusher..."
					sleep(10)
					view(6) << "<font color = green>[src]:</font> Ball!!!"
					sleep(15)
					view(6) << "<font color = green>[src]:</font> HA!!!!!!"
					src.overlays -= cb_charge
					missile('CrusherBall.dmi', src, M)
					spawn(5) src.doing = 0
					spawn(5) src.frozen = 0
					spawn(150) src.cb_lock = 0

					if(prob(7))
						M.powerlevel = 0
						M << "\blue [usr.name]'s Crusher Ball Blows you apart!"
						...
						M << "\red [usr.name]'s Crusher Ball plows into you"
						M.SmallKiDeathCheck(src)
						src.doing = 0
						src.frozen = 0
						return
