mob/var/learn_hell_impact = 0
mob/var/hi_charge = 'Hell Impact Charge.dmi'
mob/learn
	verb
		Hell_Impact()
		...
		HI_Shoot()
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					src.doing = 1
					src.ki -= ki_cost
					src.overlays += hi_charge
					view(6) << "<font color = white>[src]:</font> Hell....."
					sleep(8)
					view(6) << "<font color = white>[src]:</font> Impact!!"
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Hell Impact slams into you!"
										src <<"\red Your Hell Impact slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Hell Impact is Absorbed by [M]!"
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											src.doing = 0
											src.frozen = 0
											...
											M << "\red You try to absorb [src]'s Hell Impact, but it is too strong!"
											src <<"\red Your Hell Impact slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Hell Impact slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
