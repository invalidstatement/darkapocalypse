...
mob
	proc
		Static_Shot_Shoot()
			var/obj/H = new/obj/Static_Shot
			var/ki_damage = round(usr.powerlevel_max * 2)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Static Shot slams into you!"
										src <<"\red Your Static Shot slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Static Shot is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Static Shot, but it is too strong!"
											src <<"\red Your Static Shot slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Static Shot slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
