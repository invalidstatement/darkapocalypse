mob/var/learn_x100_kame = 0
mob/var/x100_kame_charge = 'x100 Kame Charge.dmi'
mob/learn
	verb
		X100_Kamehameha()
...
mob
	proc
		x100_Kame_Shoot()
			var/obj/H = new/obj/x100_kame
			var/ki_damage = round(usr.powerlevel_max * 1)
			var/ki_cost = round(src.ki_max * 0.90)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> X100 Kame....."
					sleep(8)
