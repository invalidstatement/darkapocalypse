...
mob
	proc
		Super_Mystic_Bomb_Shoot()
			var/obj/H = new/obj/Super_Mystic_Bomb_Blast
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					sleep(8)
					view(6) << "<font color = white>[src]:</font> Mystic!!!"
					sleep(8)
					view(6) << "<font color = white>[src]:</font> Bomb!!!"
					src.overlays -= kamehameha_charge
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Super Mystic Bomb slams into you!"
										src <<"\red Your Super Mystic Bomb slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Super Mystic Bomb is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Super Mystic Bomb, but it is too strong!"
											src <<"\red Your Super Mystic Bomb slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Super Mystic Bomb slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
...
obj/Super_Mystic_Bomb_Blast
	icon = 'Super Mystic Bomb.dmi'
