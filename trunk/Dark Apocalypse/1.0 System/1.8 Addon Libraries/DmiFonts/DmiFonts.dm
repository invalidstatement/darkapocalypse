...
// upgrade .dm file if called for
	...
	// a null in the list means following lines will be rendered in this font
	...
	proc/GetLinesMultiFont(list/items, width=-1, flags=0, firstline=0, maxheight=-1, list/leftover)
		...
		// if only a font was added (because of the previous line) to leftover,
		// in which case nothing was really left over, ax it. Also if it's
		// immediately followed by some other font, ax it
		if(leftover)
	...
	proc/DrawTextMultiFont(items, x, y, width=-1, flags=0, firstline=0, maxheight=-1, icons_x=0, icons_y=0, iconset/drawover, list/leftover)
		...
		/*
			You must explicitly delete the /dmifonttextline datums, because the
			garbage collector won't delete double-linked lists. If you supply the
			datum to the proc manually, then it's up to you to delete it. If it's
			created from a list, the proc will delete it for you.
		 */
