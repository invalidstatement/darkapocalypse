			...
			if(src in usr:contents)
				if(!usr.swordd_eq)
					if(usr.weapon == "EMPTY")
						...
						usr << "Error:</b> You already have something equipped!"
						return
				else
		...
		verb
			...
			Drop()
				...
				if(!src.equiped)
					...
					usr << "You are unable to drop: <u>[src.]</u>."