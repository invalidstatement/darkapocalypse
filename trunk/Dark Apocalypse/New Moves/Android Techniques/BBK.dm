...
mob
	proc
		Big_Bang_Kamehameha_Shoot()
			var/obj/H = new/obj/Big_Bang_Kamehameha_Blast
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Big Bang."
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Big Bang Kamehameha slams into you!"
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Big Bang Kamehameha is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Big Bang Kamehameha, but it is too strong!"
											src <<"\red Your Big Bang Kamehameha slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Big Bang Kamehameha slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
