mob/learn/
	verb
		Big_Bang_Kamehameha_X100()
			...
			Big_Bang_Kamehameha_X100_Shoot()
			src.afk_time = 0
...
mob
	proc
		Big_Bang_Kamehameha_X100_Shoot()
			var/obj/H = new/obj/Big_Bang_Kamehameha_X100_Blast
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Lets See If You Survive this one."
					sleep(8)
					view(6) << "<font color = white>[src]:</font> x100!!!"
					src.overlays -= kamehameha_charge
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Big Bang Kamehameha x100 slams into you!"
										src <<"\red Your Big Bang Kamehameha x100 slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Big Bang Kamehameha x100 is Absorbed by [M]!"
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.gooda+=1
											M.UTBA()
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Big Bang Kamehameha x100, but it is too strong!"
											src <<"\red Your Big Bang Kamehameha slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
											return

								M << "\red [src.name]'s Big Bang Kamehameha x100 slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
...
obj/Big_Bang_Kamehameha_X100_Blast
	icon = 'LG3.dmi'
