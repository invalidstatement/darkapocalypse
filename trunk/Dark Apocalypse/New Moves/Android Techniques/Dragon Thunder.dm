mob/learn/
	verb
		Dragon_Thunder(mob/M in oview(6))
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Dragon Thunder"
					sleep(15)
					view(6) << "<font color = white>[src]:</font> HA!!!!!!"
					src.overlays -= ray_charge
					missile('LG7.dmi', src, M)
					spawn(5) src.doing = 0
					...
					if(prob(7))
						M.powerlevel = 0
						M << "\blue [usr.name]'s Dragon Thunder Blows through your Chest!"
						...
						M << "\red [usr.name]'s Dragon Thunder plows into you"
						M.SmallKiDeathCheck(src)
						src.doing = 0
						src.frozen = 0
						return
