...
mob/learn
	verb/Freezing_Corkscrew(mob/M in oview(6))
		...
		var/ki_damage = round(((usr.ki_skill / usr.ki_skill_max) * usr.level) + usr.powerlevel_max * 150)
		...
		if(src.ki >= ki_cost)
			if(!src.doing)
				M.frozen = 1
				M.powerlevel /= 2
				M.underlays +=big_Freezing_Corkscrew
				src.ki_lock = 1
				spawn(70) src.ki_lock = 0
				src.doing = 1
				src.ki -= ki_cost
				src.overlays += ray_charge
				view(6) << "<font color = blue>[src]:</font> Time for you to meet your doom!!!!!!"
				sleep(10)
				view(6) << "<font color = blue>[src]:</font> Freezing CORKSCREW!!!"
				sleep(15)
				...
				if(prob(7))
					M.powerlevel = 0
					M << "\red [usr.name]'s Freezing Corkscrew Blows through your Chest!"
					...
					M << "\red [usr.name]'s Freezing Corkscrew plows into you"
					M.SmallKiDeathCheck(src)
					src.doing = 0
					src.frozen = 0
					return
