...
mob
	proc
		Dragon_Fist_Shoot()
			var/obj/H = new/obj/Dragon_Fist
			var/ki_damage = round(usr.powerlevel_max * 0.85)
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font>Dragon Fist Explode"
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Dragon Fist slams into you!"
										src <<"\red Your Dragon Fist slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Dragon Fist is Absorbed by [M]!"
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											src.doing = 0
											src.frozen = 0
											...
											M << "\red You try to absorb [src]'s Dragon Fist, but it is too strong!"
											src <<"\red Your Dragon Fist slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Dragon Fist slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
