...
mob
	proc
		DS_Shoot()
			var/obj/H = new/obj/dimension_shatter
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Dimension Shatter slams into you!"
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											M << "\red You try to absorb [src]'s Dimension Shatter, but it is too strong!"
											src <<"\red Your Dimension Shatter slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
