mob/var/ht_charge = 'HyperTornadoCharge.dmi'
mob/learn
	verb
		Hyper_Tornado()
...
mob
	proc
		Hyper_Tornado_Shoot()
			...
			var/ki_damage = round(usr.powerlevel_max * 14)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Hyper Tornado slams into you!"
										src <<"\red Your Hyper Tornado slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Hyper Tornado is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Hyper Tornado, but it is too strong!"
											src <<"\red Your Hyper Tornado slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Hyper Tornado slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
