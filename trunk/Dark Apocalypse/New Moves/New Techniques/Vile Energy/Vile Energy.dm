mob/var/learn_vile_energy = 0
mob/var/vile_energy_charge = 'Vile Energy.dmi'
mob/learn
	verb
		Vile_Energy()
		...
		Vile_Energy_Shoot()
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					src.doing = 1
					src.ki -= ki_cost
					src.overlays += vile_energy_charge
					view(6) << "<font color = white>[src]:</font> Vile....."
					sleep(8)
					view(6) << "<font color = white>[src]:</font> Energy!!"
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Vile Energy slams into you!"
										src <<"\red Your Vile Energy slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Vile Energy is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Vile Energy, but it is too strong!"
											src <<"\red Your Vile Energy slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Vile Energy slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
