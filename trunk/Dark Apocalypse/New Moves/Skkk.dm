mob/learn/
	verb
		Super_Kalo_Ken_Kamehameha_Wave()
			...
			Super_Kalo_Ken_Kamehameha_Shoot()
			src.afk_time = 0
...
mob
	proc
		Super_Kalo_Ken_Kamehameha_Shoot()
			var/obj/H = new/obj/Super_Kalo_Ken_Kamehameha_Blast
			...
			if(src.ki >= ki_cost)
				if(!src.doing)
					...
					view(6) << "<font color = white>[src]:</font> Super Kalo Ken"
					sleep(8)
							...
							if(!M.dead && !M.safe && M.pk)
								if(M.ki_shield)
									if(ki_absorbed >= 1)
										...
										M << "\red [src]'s Super Kalo Ken Kamehameha slams into you!"
										src <<"\red Your Super Kalo Ken Kamehameha slams into [M]!"
										Explode(new /Effect/BasicBoom(M.loc,1,3))
										M.powerlevel -= round(ki_absorbed)
										M.ki_shield_strength -= round(ki_damage)
										M.BigKiDeathCheck(src)
									...
									if(M.absorbing)
										if(absorb_max > ki_damage)
											...
											src <<"\red Your Super Kalo Ken Kamehameha is Absorbed by [M]!"
											M.gooda+=1
											M.UTBA()
											Explode(new /Effect/BasicBoom(M.loc,1,3))
											M.ki += ki_damage
											...
											M << "\red You try to absorb [src]'s Super Kalo Ken Kamehameha, but it is too strong!"
											src <<"\red Your Super Kalo Ken Kamehameha slams into [M]!"
											M.BigKiDeathCheck(src)
											src.doing = 0
											src.frozen = 0
								...
								M << "\red [src.name]'s Super Kalo Ken Kamehameha slams into you"
								Explode(new /Effect/BasicBoom(M.loc,1,3))
...
obj/Super_Kalo_Ken_Kamehameha_Blast
	icon = 'Super Kalo Ken Kamehameha.dmi'
